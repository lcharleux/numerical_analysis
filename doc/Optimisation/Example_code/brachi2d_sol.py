...
# MAILLAGE EN X 
Np = 2 # Nombre de noeuds souhaites
...
# AFFICHAGE
N = 100
yp = np.linspace(-.5, .9,N )
Yp0, Yp1 = np.meshgrid(yp, yp) 
Yp0 = Yp0.flatten()
Yp1 = Yp1.flatten()

t =np.zeros_like(Yp0)
for i in xrange(len(Yp0)):
  Y[1] = Yp0[i]
  Y[2] = Yp1[i]
  t[i] = temps(Y)
...
Y[1] = Yp0[loc]
Y[2] = Yp1[loc]
plt.plot(X, Y, "ob-", label = "$t_f = {0:.2f}$ s".format(t[loc][0]))
plt.legend()
plt.grid()
plt.xlabel("Position, $x$")
plt.ylabel("Position, $y$")
plt.savefig("brachi2d_sol.pdf")  
